...
export default function Dashboard() {
  const {
    ...
    config, categories, vipPrices, topUpPackages, flashSale, pendingTopUps, pendingWithdrawals, pendingSellerApplications, notifications,
    setCurrentUser, updateUser, updatePoints, updateStat, addNotification, setDB,
    addToCart, removeFromCart, updateCartQty, toggleFavorite, toggleCompare,
    purchaseVIP, requestTopUp, approveTopUp, rejectTopUp, requestWithdraw, approveWithdraw, rejectWithdraw,
    submitSellerApplication, approveSellerApplication, rejectSellerApplication,
    submitReview, completeTask, playGame, spinWheel, sendTS, requestReturn, approveReturn, rejectReturn,
    addProduct, deleteProduct, boostProduct, banUser, setUserRole, exportData, deleteAccount,
    addAnnouncement, updateConfig, updateCategories, updateVipPrices, updateTopUpPackages
  } = useStore();
  ...
  const filteredProducts = useMemo(() => {    let result = products.filter(p => p.status === 'ACTIVE');
    ...
    result = result.filter(p => {
      ...
      const matchesSubcat = selectedSubcat === "all" || p.subcat === selectedSubcat;
  ...
  const handleCheckout = async (couponCode: string | null) => {
    ...
    try {
      ...
      if (cleanCoupon) {
        ...
        const sub = cart.reduce((acc, item) => acc + (item.price * item.qty), 0);        discount = coupon.type === 'flat' ? coupon.value : Math.floor(sub * (coupon.value / 100));
        usedCoupon = coupon;
      }
      const subtotal = cart.reduce((acc, item) => acc + (item.price * item.qty), 0);
      let bulkDiscount = 0;
      const totalItems = cart.reduce((acc, item) => acc + item.qty, 0);
      if (totalItems >= 3) bulkDiscount = Math.floor(subtotal * 0.05);
      let flashDiscount = 0;
      ...
      setDB(prev => {
        ...
        const updatedUsers = prev.users.map(u => {          if (u.id === user.id) return { ...u, points: newPoints, stats: { ...u.stats, spent: (u.stats.spent || 0) + total, orders: (u.stats.orders || 0) + 1 } };
        ...
        if (usedCoupon) updatedCoupons = prev.coupons.map(c => c.code === usedCoupon.code ? { ...c, usedCount: c.usedCount + 1 } : c);
    ...
    return (
      <div onClick={() => setProductDetail(product)} className={cn("bg-slate-900 border rounded-2xl overflow-hidden cursor-pointer hover:scale-[1.02] transition-all group", isBoosted ? 'border-yellow-500/70 shadow-lg shadow-yellow-500/20' : 'border-slate-800 hover:border-blue-500/50')}>
        <div className="relative">
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleCompare(product.id); }} className={cn("absolute top-14 right-3 z-10 p-2 backdrop-blur rounded-full transition-colors", isCompared ? 'bg-blue-500 text-white' : 'bg-slate-900/80 text-white hover:bg-slate-800')}><Icon name="compare" className="w-4 h-4" /></button>
  ...
  const GamesView = () => {
    ...
    const stats = { totalGames: myHistory.length, won: myHistory.filter(g => g.won).length, lost: myHistory.filter(g => !g.won).length, profit: myHistory.reduce((sum, g) => sum + (g.profit || 0), 0) };    return (
      <div className="space-y-6">
  ...
  const LeaderboardView = () => (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-white">🏆 Liderlik Tablosu</h2>
  ...
  const ProfileView = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {activeTab === 'security' && (<div className="space-y-4"><div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><div className="flex justify-between items-center mb-3"><div><h3 className="text-lg font-bold text-white">2FA (İki Faktörlü)</h3><p className="text-sm text-slate-400">Telegram üzerinden kod</p></div><RippleButton onClick={() => { updateUser(user.id, { twoFA: !user.twoFA }); notify(user.twoFA ? "2FA devre dışı" : "2FA aktif", "info"); }} variant={user.twoFA ? 'danger' : 'success'}>{user.twoFA ? 'Kapat' : 'Aç'}</RippleButton></div></div><div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-3">KVKK / GDPR</h3><div className="flex gap-3"><RippleButton variant="outline" onClick={exportData}><Icon name="download" className="w-4 h-4" /> Verileri İndir</RippleButton><RippleButton variant="danger" onClick={deleteAccount}><Icon name="trash" className="w-4 h-4" /> Hesabı Sil</RippleButton></div></div></div>)}
      </div>    );
  };

  const SellerApplicationView = () => {
    const [form, setForm] = useState({ fullName: '', phone: '', city: '', birthDate: '', reason: '', telegramUsername: user.telegramUsername || '', acceptContract: false });
    const handleSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      if (!form.fullName || form.fullName.length < 3) return notify("Geçerli ad soyad girin", "error");
      if (!form.reason || form.reason.length < 100) return notify("Sebep en az 100 karakter olmalı", "error");
      if (!form.acceptContract) return notify("Sözleşmeyi kabul etmelisin", "error");
      submitSellerApplication(form);
      setView('profile');
    };
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        <h2 className="text-3xl font-bold text-white">🏪 Satıcı Başvurusu</h2>
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="text-sm text-slate-400">Ad Soyad *</label><input type="text" value={form.fullName} onChange={e => setForm({...form, fullName: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" required /></div>
            <div><label className="text-sm text-slate-400">Telefon *</label><input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" required /></div>
            <div><label className="text-sm text-slate-400">Şehir *</label><input type="text" value={form.city} onChange={e => setForm({...form, city: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" required /></div>
            <div><label className="text-sm text-slate-400">Doğum Tarihi *</label><input type="date" value={form.birthDate} onChange={e => setForm({...form, birthDate: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" required /></div>
            <div><label className="text-sm text-slate-400">Neden Satıcı Olmak İstiyorsun? (min 100 karakter) *</label><textarea value={form.reason} onChange={e => setForm({...form, reason: e.target.value})} rows={4} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" required /><p className="text-xs text-slate-500 mt-1">{form.reason.length}/100</p></div>
            <label className="flex items-start gap-2 text-sm text-slate-400 cursor-pointer"><input type="checkbox" checked={form.acceptContract} onChange={e => setForm({...form, acceptContract: e.target.checked})} className="mt-1" required /><span>Sözleşme şartlarını kabul ediyorum. *</span></label>
            <RippleButton type="submit" size="lg" className="w-full">Başvuruyu Gönder</RippleButton>
          </form>
        </div>
      </div>
    );
  ...
  const SellerDashboard = () => {
    if (!user.isSeller) return (<div className="text-center py-16 text-slate-500"><Icon name="lock" className="w-16 h-16 mx-auto mb-4 opacity-50" /><h3 className="text-xl font-bold text-white mb-2">Satıcı Değilsiniz</h3><RippleButton onClick={() => setView('seller-application')}>Başvuru Yap</RippleButton></div>);
    const myProducts = products.filter(p => p.sellerId === user.id);
    const [showAdd, setShowAdd] = useState(false);
    const [newProd, setNewProd] = useState({ name: '', price: '', category: categories[0]?.id || '', inventory: '', desc: '', tags: '' });
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center"><h2 className="text-3xl font-bold text-white">🏪 Satıcı Paneli</h2><RippleButton onClick={() => setShowAdd(!showAdd)} variant={showAdd ? "secondary" : "primary"}>{showAdd ? 'İptal' : '+ Yeni Ürün'}</RippleButton></div>
        {showAdd && (<div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-4">Yeni Ürün Ekle</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="text-sm text-slate-400">Ürün Adı *</label><input type="text" value={newProd.name} onChange={e => setNewProd({...newProd, name: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg p-2 text-white" /></div><div><label className="text-sm text-slate-400">Fiyat (TS) *</label><input type="number" value={newProd.price} onChange={e => setNewProd({...newProd, price: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg p-2 text-white" /></div><div className="md:col-span-2"><label className="text-sm text-slate-400">Stok Kodları (her satıra bir tane) *</label><textarea value={newProd.inventory} onChange={e => setNewProd({...newProd, inventory: e.target.value})} rows={4} className="w-full mt-1 bg-slate-950 border border-slate-700 rounded-lg p-2 text-white font-mono text-xs" /></div></div><RippleButton onClick={() => { addProduct(newProd); setShowAdd(false); setNewProd({ name: '', price: '', category: categories[0]?.id || '', inventory: '', desc: '', tags: '' }); }} className="w-full mt-4" size="lg">Ürünü Yayınla</RippleButton></div>)}
  ...
  const AdminPanel = () => {
    if (!hasPermission(user.role, 3)) return (<div className="text-center py-16 text-slate-500"><Icon name="lock" className="w-16 h-16 mx-auto mb-4 opacity-50" /><h3 className="text-xl font-bold text-white mb-2">Erişim Yok</h3></div>);
    const [adminTab, setAdminTab] = useState('overview');
    const openTickets = tickets.filter(t => t.status === 'OPEN');    return (
      <div className="space-y-6">
        <h2 className="text-3xl font-bold text-white">⚙️ Yönetim Paneli</h2>
        <div className="flex gap-2 overflow-x-auto no-scrollbar">{[{ id: 'overview', label: 'Genel', level: 3 }, { id: 'pending', label: `Bekleyen (${pendingTopUps.length + pendingWithdrawals.length + pendingSellerApplications.filter(a=>a.status==='PENDING').length})`, level: 4 }, { id: 'tickets', label: `İade (${openTickets.length})`, level: 4 }, { id: 'users', label: 'Kullanıcılar', level: 4 }].filter(t => hasPermission(user.role, t.level)).map(tab => (<button key={tab.id} onClick={() => setAdminTab(tab.id)} className={cn("px-4 py-2 rounded-lg font-medium whitespace-nowrap", adminTab === tab.id ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400')}>{tab.label}</button>))}</div>
        
        {adminTab === 'overview' && (<div className="grid grid-cols-2 md:grid-cols-4 gap-3"><div className="bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-400">Toplam Ürün</p><p className="text-2xl font-bold text-white">{products.length}</p></div><div className="bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-400">Toplam Sipariş</p><p className="text-2xl font-bold text-white">{orders.length}</p></div><div className="bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-400">Toplam Satış</p><p className="text-2xl font-bold text-emerald-400">{formatNumber(orders.reduce((sum, o) => sum + o.total, 0))} TS</p></div><div className="bg-slate-900 border border-slate-800 rounded-xl p-4"><p className="text-xs text-slate-400">Kullanıcı</p><p className="text-2xl font-bold text-white">{users.length}</p></div></div>)}
        
        {adminTab === 'pending' && (
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-4">💰 Bekleyen Bakiye Yüklemeleri ({pendingTopUps.length})</h3><div className="space-y-3">{pendingTopUps.length === 0 && <p className="text-center text-slate-500 py-8">Bekleyen yok</p>}{pendingTopUps.map(t => (<div key={t.id} className="p-4 bg-slate-800/50 rounded-xl"><div className="flex justify-between items-start mb-2"><div><p className="font-bold text-white">{t.username}</p><p className="text-xs text-slate-400">{t.amount} TS • ₺{t.price}</p></div><Badge variant="warning">Beklemede</Badge></div><div className="flex gap-2"><RippleButton variant="danger" size="sm" onClick={() => rejectTopUp(t.id, 'Reddedildi')}>Reddet</RippleButton><RippleButton variant="success" size="sm" onClick={() => approveTopUp(t.id)}>✓ Onayla</RippleButton></div></div>))}</div></div>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-4">🏪 Satıcı Başvuruları ({pendingSellerApplications.filter(a => a.status === 'PENDING').length})</h3><div className="space-y-3">{pendingSellerApplications.filter(a => a.status === 'PENDING').map(a => (<div key={a.id} className="p-4 bg-slate-800/50 rounded-xl"><div className="flex justify-between items-start mb-2"><div><p className="font-bold text-white">{a.username}</p><p className="text-xs text-slate-400">{a.fullName} • {a.city}</p></div><Badge variant="warning">Beklemede</Badge></div><div className="flex gap-2"><RippleButton variant="danger" size="sm" onClick={() => rejectSellerApplication(a.id, 'Uygun bulunmadı')}>Reddet</RippleButton><RippleButton variant="success" size="sm" onClick={() => approveSellerApplication(a.id)}>✓ Onayla</RippleButton></div></div>))}</div></div>
          </div>
        )}

        {adminTab === 'tickets' && (<div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-4">İade Talepleri</h3><div className="space-y-3">{openTickets.length === 0 && <p className="text-center text-slate-500 py-8">Bekleyen talep yok.</p>}{openTickets.map(t => (<div key={t.id} className="p-4 bg-slate-800/50 rounded-xl"><div className="flex justify-between items-start mb-2"><div><p className="font-bold text-white">{t.itemName}</p><p className="text-xs text-slate-400">{t.orderId}</p></div><Badge variant="warning">Açık</Badge></div><p className="text-sm text-slate-300 mb-3">Sebep: {t.reason}</p><div className="flex gap-2"><RippleButton variant="danger" size="sm" onClick={() => rejectReturn(t.id)}>Reddet</RippleButton><RippleButton variant="success" size="sm" onClick={() => approveReturn(t.id)}>Onayla</RippleButton></div></div>))}</div></div>)}
        
        {adminTab === 'users' && (<div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-lg font-bold text-white mb-4">Kullanıcılar ({users.length})</h3><div className="space-y-2 max-h-[600px] overflow-y-auto scrollbar-thin">{users.map(u => (<div key={u.id} className="p-3 bg-slate-800/50 rounded-lg"><div className="flex justify-between items-start gap-2 flex-wrap"><div className="flex-1 min-w-0"><p className="font-bold text-white">{u.username} {u.banned && <Badge variant="danger" className="ml-2">BANLI</Badge>}</p><p className="text-xs text-slate-400 capitalize">{u.role} • Lv.{u.level} • {formatNumber(u.points)} TS</p></div><div className="flex gap-1 flex-wrap">{hasPermission(user.role, 4) && u.role !== ROLES.FOUNDER && ROLE_HIERARCHY[u.role] < ROLE_HIERARCHY[user.role] && (<RippleButton onClick={() => banUser(u.id)} size="xs" variant={u.banned ? 'success' : 'danger'}>{u.banned ? 'Yasak Kaldır' : 'Banla'}</RippleButton>)}</div></div></div>))}</div></div>)}
      </div>
    );
  };
    ...
    <div className="flex h-screen bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 lg:ml-64 overflow-y-auto scrollbar-thin pb-20 lg:pb-4">
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-slate-900/95 backdrop-blur border-b border-slate-800 p-4">
          <div className="flex justify-between items-center"><div className="flex items-center gap-2"><button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-white p-2"><Icon name={isMobileMenuOpen ? "x" : "menu"} className="w-6 h-6" /></button><h1 className="text-lg font-bold text-white">{config.siteName}</h1></div><div className="flex gap-2"><button onClick={() => setIsNotifOpen(true)} className="relative p-2 text-white"><Icon name="bell" className="w-5 h-5" />{unreadNotifs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full"></span>}</button><button onClick={() => setIsCartOpen(true)} className="relative p-2 text-white"><Icon name="bag" className="w-5 h-5" />{cart.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{cart.reduce((a,c)=>a+c.qty,0)}</span>}</button></div></div>
        ...
        <main className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          {view === 'seller-application' && <SellerApplicationView />}
      ...
      {isNotifOpen && (<>
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={() => setIsNotifOpen(false)} />
        <div className="fixed top-0 right-0 bottom-0 w-full md:w-96 bg-slate-900 border-l border-slate-800 z-50 overflow-y-auto scrollbar-thin flex flex-col">
          <div className="p-4 border-b border-slate-800 flex justify-between items-center sticky top-0 bg-slate-900"><h3 className="text-xl font-bold text-white">🔔 Bildirimler</h3><button onClick={() => setIsNotifOpen(false)} className="text-slate-400 hover:text-white p-1"><Icon name="x" className="w-6 h-6" /></button></div>
          <div className="flex-1 p-4 space-y-3">{notifications.length === 0 ? (<div className="text-center py-12 text-slate-500"><p>Bildirim yok</p></div>) : notifications.map(n => (<div key={n.id} className={cn("p-3 rounded-xl border", n.read ? 'bg-slate-800/30 border-slate-800 opacity-60' : 'bg-slate-800/50 border-slate-700')}><div className="flex justify-between items-start mb-1"><Badge variant={n.type === 'success' ? 'success' : n.type === 'error' ? 'danger' : 'info'}>{n.type}</Badge><span className="text-xs text-slate-500">{timeAgo(n.date)}</span></div><p className="text-sm text-white">{n.msg}</p></div>))}</div>
        </div>
      </>)}
      ...
      <Modal isOpen={showVIP} onClose={() => setShowVIP(false)} title="👑 VIP Üyelik" size="lg">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{Object.values(VIP_LEVELS).filter(l => l !== VIP_LEVELS.NONE).map(level => { const benefits = VIP_BENEFITS[level as keyof typeof VIP_BENEFITS]; const prices = vipPrices[level as keyof typeof vipPrices]; return (<div key={level} className="bg-slate-800/50 rounded-2xl p-5 text-center"><div className="text-5xl mb-3">{benefits.icon}</div><h3 className="text-xl font-bold text-white mb-2">{benefits.name}</h3><div className="space-y-2 text-sm text-slate-300 mb-4"><p>🎁 Aylık Hediye: {benefits.monthlyGift} TS</p><p>💸 Transfer Vergisi: %{(benefits.transferTaxRate * 100).toFixed(1)}</p></div><div className="space-y-2"><RippleButton variant="gold" size="sm" className="w-full" onClick={() => purchaseVIP(level, 'weekly')}>Haftalık: {formatNumber(prices.weekly)} TS</RippleButton><RippleButton variant="gold" size="sm" className="w-full" onClick={() => purchaseVIP(level, 'monthly')}>Aylık: {formatNumber(prices.monthly)} TS</RippleButton></div></div>); })}</div>
      </Modal>

      {/* Compare Modal */}
      <Modal isOpen={showCompare} onClose={() => setShowCompare(false)} title="📊 Ürün Karşılaştırma" size="lg">
        <div className="overflow-x-auto"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{compareList.map(id => { const p = products.find(pr => pr.id === id); if (!p) return null; return (<div key={id} className="bg-slate-800/50 rounded-2xl p-4"><img src={p.image} alt={p.name} className="w-full h-32 object-cover rounded-lg mb-3" /><h4 className="font-bold text-white mb-2">{p.name}</h4><div className="space-y-1 text-sm"><div className="flex justify-between"><span className="text-slate-400">Fiyat</span><span className="text-blue-400">{formatNumber(p.price)} TS</span></div><div className="flex justify-between"><span className="text-slate-400">Stok</span><span>{p.stock?.length || 0}</span></div></div><button onClick={() => toggleCompare(p.id)} className="w-full mt-3 text-red-400 text-sm">Kaldır</button></div>); })}</div></div>
      </Modal>
...
// Helper component for Send TS form
