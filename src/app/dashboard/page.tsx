...
export default function Dashboard() {
  const {
    ...
    config, categories, vipPrices, topUpPackages, flashSale, pendingTopUps, pendingWithdrawals, pendingSellerApplications,
    ...
    return (
      <div onClick={() => setProductDetail(product)} className={cn("bg-slate-900 border rounded-2xl overflow-hidden cursor-pointer hover:scale-[1.02] transition-all group", isBoosted ? 'border-yellow-500/70 shadow-lg shadow-yellow-500/20' : 'border-slate-800 hover:border-blue-500/50')}>
        <div className="relative">
          ...
          <button onClick={(e) => { e.stopPropagation(); useStore.getState().toggleCompare(product.id); }} className={cn("absolute top-14 right-3 z-10 p-2 backdrop-blur rounded-full transition-colors", isCompared ? 'bg-blue-500 text-white' : 'bg-slate-900/80 text-white hover:bg-slate-800')}><Icon name="compare" className="w-4 h-4" /></button>
  ...
  return (
    <div className="flex h-screen bg-slate-950 overflow-hidden">
      ...
      <Modal isOpen={showCompare} onClose={() => setShowCompare(false)} title="📊 Ürün Karşılaştırma" size="lg">
        <div className="overflow-x-auto"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{compareList.map(id => { const p = products.find(pr => pr.id === id); if (!p) return null; return (<div key={id} className="bg-slate-800/50 rounded-2xl p-4"><img src={p.image} alt={p.name} className="w-full h-32 object-cover rounded-lg mb-3" /><h4 className="font-bold text-white mb-2">{p.name}</h4><div className="space-y-1 text-sm"><div className="flex justify-between"><span className="text-slate-400">Fiyat</span><span className="text-blue-400">{formatNumber(p.price)} TS</span></div><div className="flex justify-between"><span className="text-slate-400">Stok</span><span>{p.stock?.length || 0}</span></div></div><button onClick={() => useStore.getState().toggleCompare(p.id)} className="w-full mt-3 text-red-400 text-sm">Kaldır</button></div>); })}</div></div>
      </Modal>
